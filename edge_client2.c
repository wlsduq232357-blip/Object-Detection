...
#define DEFAULT_SEGMENT_DIR   "/home/user/segments"
...
static int start_ffmpeg(void) {
    ...
    if (pid == 0) {
        ...
        int fps = 30;
        int seg = g_cfg.segment_time_sec;   // cfg의 10
        int gop = fps * seg;                // 10초마다 키프레임

        snprintf(cmd, sizeof(cmd),
                 FFMPEG_CMD
                 " -f v4l2 -input_format mjpeg -video_size 640x480 -framerate %d -i %s "
                 " -c:v libx264 -preset veryfast -tune zerolatency -pix_fmt yuv420p "
                 " -r %d -g %d -keyint_min %d -sc_threshold 0 -bf 0 "
                 " -force_key_frames \"expr:gte(t,n_forced*%d)\" "
                 " -f tee \"[f=mpegts]udp://%s:%d|"
                 "[f=segment:segment_time=%d:reset_timestamps=1:strftime=1:segment_format=mp4]%s/cam01_%%%%Y%%%%m%%%%d_%%%%H%%%%M%%%%S.mp4\"",
                 fps, g_cfg.camera_device,
                 fps, gop, gop, seg,
                 g_cfg.server_ip, g_cfg.udp_port,
                 seg, g_cfg.segment_dir);
